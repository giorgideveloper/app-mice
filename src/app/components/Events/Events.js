...
export default function Events({ dict, events, lang, category }) {
    ...
    const eventsFilterDate = async () => {
      try {
        if (selectedStartDate && selectedEndDate) {
          ...
          const data = await fetchEventsDate(
            lang,
            category,
            tagToFilter,
            selectedStartDate,
  ...
  useEffect(() => {
    if (
      selectedStartDate &&
      selectedEndDate &&
      (filterTag || filteredByDateEvents.length > 0)
    ) {
      ...
      filterTag &&
      (!filterTagEvents || filterTagEvents.length === 0)
    ) {
      setDisplayedEvents([]);
    } else if (
      selectedStartDate &&
      selectedEndDate &&
      filteredByDateEvents.length === 0
    ) {
      setDisplayedEvents([]);
    } else {
